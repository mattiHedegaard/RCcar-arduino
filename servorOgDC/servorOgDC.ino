...
// This function controles the speed of the motor
void DirectionControle(int acDelay, bool forwards) {
  	...
  	else
    {
  	...
  	while (digitalRead(fwdBtn) || digitalRead(bwdBtn)){
  	...
  	Deccelerate(10,currentSpeed);
}

void Deccelerate(int delayTime, int motorSpeed){  
    for (int i = motorSpeed; i >= 0; i--) {
     	if (digitalRead(fwdBtn) == HIGH || digitalRead(bwdBtn) == HIGH){
